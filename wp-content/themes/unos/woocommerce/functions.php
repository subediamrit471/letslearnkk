 ...
 * @return void
 */
if ( !function_exists( 'unos_woo_setup' ) ) :
function unos_woo_setup() {
 ...
 * @return void
 */
if ( !function_exists( 'unos_woo_register_sidebars' ) ) :
function unos_woo_register_sidebars() {
	hoot_register_sidebar(
		array(
			'id'          => 'hoot-woo-sidebar-primary',
			'name'        => _x( 'Woocommerce Primary Sidebar', 'sidebar', 'unos' ),
			'description' => __( 'The primary sidebar for woocommerce pages.', 'unos' )
		)
	);
	hoot_register_sidebar(
		array(
			'id'          => 'hoot-woo-sidebar-secondary',
			'name'        => _x( 'Woocommerce Secondary Sidebar', 'sidebar', 'unos' ),
			'description' => __( 'The secondary sidebar for woocommerce pages (if you are using a 3 column layout with 2 sidebars).', 'unos' )
		)
	);
}
endif;
add_action( 'widgets_init', 'unos_woo_register_sidebars' );
 ...
 * @return array
 */
if ( !function_exists( 'unos_woo_attr_sidebar' ) ) :
function unos_woo_attr_sidebar( $attr, $context ) {
...
add_filter( 'hoot_attr_sidebar', 'unos_woo_attr_sidebar', 11, 2 );
 ...
 * @return array
 */
if ( !function_exists( 'unos_woo_layout' ) ) :
function unos_woo_layout( $sidebar ) {
	...
	$forcenosidebar = apply_filters( 'unos_woo_pages_force_nosidebar', true );
...
add_filter( 'unos_layout', 'unos_woo_layout' );
 ...
 * @param string $context
 */
if ( !function_exists('unos_woo_meta_info_blocks_display') ) :
function unos_woo_meta_info_blocks_display( $display, $context ) {
	if ( is_woocommerce() || is_cart() || is_checkout() || is_account_page() )
		$display = array();
	return $display;
}
endif;
add_filter( 'hoot_meta_info', 'unos_woo_meta_info_blocks_display', 10, 2 );
 ...
 * @return void
 */
if ( !function_exists( 'unos_woo_loop_hide_product_meta' ) ) :
function unos_woo_loop_hide_product_meta() {
