...
function hoot_get_wp_link() {
	return sprintf( '<a class="wp-link" href="%s">%s</a>', esc_url( __( 'https://wordpress.org', 'unos' ) ), esc_html__( 'WordPress', 'unos' ) );
}
endif;
...
function hoot_display_meta_info( $args = '', $context = '', $editlink = true ) {
	...
	if ( !empty( $display['author'] ) ) :
		$blocks['author']['label'] = __( 'By:', 'unos' );
		ob_start();
	...
	if ( !empty( $display['cats'] ) ) :
		...
		if ( !empty( $category_list ) ) :
			$blocks['cats']['label'] = __( 'In:', 'unos' );
			$blocks['cats']['content'] = $category_list;
		endif;
	endif;

	if ( !empty( $display['tags'] ) && get_the_tags() ) :
		$blocks['tags']['label'] = __( 'Tagged:', 'unos' );
		$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'unos' ) : get_the_tag_list( '', ', ', '' );
	endif;
	...
	if ( $editlink && $edit_link = get_edit_post_link() ) :
		...
		$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'unos' ) . '</a>';
	endif;
...
function hoot_developer_data( $commented = true ) {
	ob_start();
	echo intval( get_num_queries() ) . ' ' . esc_attr__( 'queries.', 'unos' ) . ' ';
	timer_stop(1);
	echo esc_html( __( ' seconds. ', 'unos' ) . ( memory_get_peak_usage(1) / 1024 ) / 1024 . 'MB' );
	$out = ob_get_contents();
	ob_end_clean();
	return $out;
}
